...
app.post('/render-video', async (req, res) => {
    ...
    try {
        ...
        await page.evaluate((project) => { document.body.style.backgroundColor = 'transparent'; document.documentElement.style.backgroundColor = 'transparent'; const mediaWrapper = document.querySelector('#st-preview-media-container-wrapper'); if (mediaWrapper) mediaWrapper.style.display = 'none'; const pSettings = project.projectSettings; if (pSettings.header.logo.url) { const img = new Image(); img.src = pSettings.header.logo.url; } }, projectData);
        let frameCount = 0;
        ...
        const finalFfmpegCommand = `ffmpeg -i "${baseVideoPath}" -i "${overlayFramesDir}/frame_%06d.png" ${audioInput} -filter_complex "[0:v][1:v]overlay=0:0:shortest=1[v]; [v]colorspace=bt709:iall=bt709:itrc=srgb[final_v]" -map "[final_v]" ${audioMap} -c:v libx264 -crf 18 -preset slow -pix_fmt yuv420p -c:a aac -y "${outputVideoPath}"`;
        console.log(`[${renderId}] Pass 2: 최종 영상 합성 실행`);
        await new Promise((resolve, reject) => exec(finalFfmpegCommand, (err) => { if (err) { console.error('최종 합성 실패:', err); reject(new Error(err)); } else resolve(); }));
        ...
        if (!res.headersSent) res.status(500).json({ success: false, message: '영상 생성 중 오류가 발생했습니다. 서버 로그를 확인해주세요.' });
    }
});
